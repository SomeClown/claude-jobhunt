---
name: render-docx
description: Use when resume.md or cover-letter.md for a job has been written or edited and needs to become a .docx a human can actually send — renders JOBS_DIR/output/<slug>/resume.md and/or cover-letter.md in place, into that same JOBS_DIR/output/<slug>/ folder, via the markdown-driven renderer in scripts/.
---
...
The `output/[slug]/` convention and the renderer invocation. It does **not**
...
writes prose; it only converts what is already on disk, in place — the
markdown source and the rendered `.docx` end up as siblings in the same
`output/[slug]/` folder.
...
`output/[slug]/resume.md` and `output/[slug]/cover-letter.md` for the canonical
...
```
node ${CLAUDE_PLUGIN_ROOT}/scripts/render.js --job <slug> --type resume|cover-letter|both [--jobs-dir <path>] [--out <path>]
```

- `--job <slug>` — the job folder under `JOBS_DIR/output/`, e.g.
  `northwind-systems-dir-it-2026-01-15`.
- `--type` — `resume`, `cover-letter`, or `both` (renders the pair in one
  call).
- `--jobs-dir <path>` — optional override. Without it, `JOBS_DIR` resolves
  the same way everywhere in this plugin: `$JOBHUNT_JOBS_DIR` →
  `./job-hunt/` → `~/job-hunt/`. See
...
`JOBS_DIR/output/<slug>/<Name>_<Type>.docx` — one file per type, alongside
the `resume.md`/`cover-letter.md` it was rendered from, e.g.
...
hardcoded. Output lives beside the rest of `JOBS_DIR`, not inside the
